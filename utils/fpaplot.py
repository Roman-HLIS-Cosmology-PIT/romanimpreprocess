...
ctrs = np.array([[2214,1215],
 ...
 [-11070,4220],
 [-11148,-698],
 [-11264,-5106]], dtype=np.int32)
bbox = {
    'xmin': -13312,
    'xmax': 13312,
    'ymin': -10254,
    'ymax':  6268
} # bounding box includes reference pixels
...
def read_sca_image(infile_format, n1, ptype, scanum, **kwargs):
    ...
    if not os.path.exists(file): return arr
    ...
    with asdf.open(file) as f:
        ...
        if s>0: use_obj = np.pad(use_obj, s)
...
def write_text(image, origin, size, val, string):
   ...
   for i in range(len(string)):
        if posx+size*6>np.shape(image)[-1] or posy+size*12>np.shape(image)[-2]: break
        card = letters[ord(string[i]),::-1,:]
        card = np.repeat(np.repeat(card, size, axis=0), size, axis=1)
        image[posy:posy+size*12,posx:posx+size*6] = np.where(card>0, val, image[posy:posy+size*12,posx:posx+size*6])
        posx += size*8
...
def make_big_image(infile_format, n1, ptype, **kwargs):
    ...
    for scanum in range(1,19):
        ...
        if ptype=='pflat': myImage /= (np.nanmedian(myImage) + 1e-24)

        myImage = np.nan_to_num(myImage,0.)
        myImage = np.clip((myImage-kwargs.get('vmin'))/(kwargs.get('vmax')-kwargs.get('vmin')),0.,1.)
    ...
    if 'scale' in kwargs:
        arr[-(n1//8):, nx//2-n1:nx//2+n1, :] = cmap(np.linspace(0,1,2*n1), bytes=True)[None,:,:3]
        sc = max(n1,64)//64
        posy = ny - n1//8 - 15*sc
        for j in range(3):
            arr[-(n1//8)-2*sc:-(n1//8),nx//2-n1+j*n1:nx//2-n1+j*n1+sc,:] = 0
            txt = kwargs.get('scale').format(j/2.*(kwargs.get('vmax')-kwargs.get('vmin'))+kwargs.get('vmin'))
            posx = nx//2-n1 + n1*j - 3*sc*len(txt)
            for l in range(3):
                write_text(arr[:,:,l], (posy,posx), sc, 0, txt)
        ...
        posy = ny - n1//8 - 27*sc
        for l in range(3):
            write_text(arr[:,:,l], (posy,posx), sc, 0, label[ptype])
...
def multi_image(infile_format, n1, masktype):
    ...
    # linearity
    my_images.append(make_big_image(infile_format,n1,'lin2',vmin=-100.,vmax=2900.,scale='{:4.0f}',mask=masktype))
    my_images.append(make_big_image(infile_format,n1,'lin3',vmin=-100.,vmax=1500.,scale='{:4.0f}',mask=masktype))

    # gain
    my_images.append(make_big_image(infile_format,n1,'gain',vmin=1.2,vmax=2.1,scale='{:4.2f}',mask=masktype))

    # IPC
    my_images.append(make_big_image(infile_format,n1,'alphaD',vmin=0.,vmax=.004,scale='{:5.3f}',mask=masktype))
    my_images.append(make_big_image(infile_format,n1,'alphaH',vmin=.005,vmax=.025,scale='{:5.3f}',mask=masktype))
    my_images.append(make_big_image(infile_format,n1,'alphaV',vmin=.005,vmax=.025,scale='{:5.3f}',mask=masktype))

    # flat
    my_images.append(make_big_image(infile_format,n1,'pflatnorm',vmin=0.8,vmax=1.2,scale='{:4.2f}',mask=masktype))
    # read noise
    my_images.append(make_big_image(infile_format,n1,'read',vmin=4.,vmax=9.,scale='{:4.1f}',mask=masktype))
