...
def get_flat(caldir, meta, pdq, ipc_deconvolve=True):
    ...
    with asdf.open(caldir['flat']) as f:
        ...
        this_flat[nborder:ny-nborder,nborder:nx-nborder] = f['roman']['data'][nborder:ny-nborder,nborder:nx-nborder]

    # this just prevents divide-by-zero warning for the bad pixels
    if pdq is not None:
        pdq |= np.where(np.logical_or(this_flat<.1,this_flat>10), pixel.NO_FLAT_FIELD, 0).astype(np.uint32)
    this_flat = np.clip(this_flat, .1, 10)
    ...
    if ipc_deconvolve:
        ...
        with asdf.open(caldir['gain']) as f:
            ...
            if pdq is not None:
                pdq[nborder:ny-nborder,nborder:nx-nborder] |= np.where(g<=.1, pixel.NO_GAIN_VALUE, 0).astype(np.uint32)
                g = np.clip(g,.1,None)
            # de-convolve the IPC kernel
            with asdf.open(caldir['ipc4d']) as ipc:
                this_flat[nborder:ny-nborder,nborder:nx-nborder] = ipc_rev(this_flat[nborder:ny-nborder,nborder:nx-nborder], ipc['roman']['data'], gain=g)
