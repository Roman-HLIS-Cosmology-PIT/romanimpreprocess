...
from romanimpreprocess.L1_to_L2.GalPoisson.draw_with_tilnus import (
    solve_beta_parameters_vec,
    solve_pearson5_parameters_vec,
    solve_pearson6_params,
)
...
def analymoms_type5(
    ...
    a, b, _ = solve_pearson5_parameters_vec(tilnu21, tilnu31, I_vals)
...
def moms_from_Pearson(
    tilnu_21: float, tilnu_31: float, tilnu_41: float, I: np.ndarray, *, atol: float = 1e-14
):
    """
    ...
    if np.any(type1):
        ...
        analymoms[type1, 0] = m2
        analymoms[type1, 1] = m3
        analymoms[type1, 2] = m4
    if np.any(type3):
        m2, m3, m4 = analymoms_type3(tilnu_21, tilnu_31, I_clipped[type3])
        analymoms[type3, 0] = m2
        analymoms[type3, 1] = m3
        analymoms[type3, 2] = m4
    if np.any(type4):
        m2, m3, m4 = analymoms_type4(tilnu_21, tilnu_31, tilnu_41, I_clipped[type4])
        analymoms[type4, 0] = m2
        analymoms[type4, 1] = m3
        analymoms[type4, 2] = m4
    if np.any(type5):
        m2, m3, m4 = analymoms_type5(tilnu_21, tilnu_31, I_clipped[type5])
        analymoms[type5, 0] = m2
        analymoms[type5, 1] = m3
        analymoms[type5, 2] = m4
    if np.any(type6):
        m2, m3, m4 = analymoms_type6(tilnu_21, tilnu_31, tilnu_41, I_clipped[type6])
        analymoms[type6, 0] = m2
        analymoms[type6, 1] = m3
        analymoms[type6, 2] = m4
...
I = np.array([5.0, 100.0, 1000.0, 10000.0, 100000.0])  # Needs to be a row matrix.

err_mat = make_err_mat(tilnu21_arr, tilnu31_arr, tilnu41_arr, I)

np.save(
    "analy_errormat.npy", err_mat
)  # complete error matrix saved as numpy array in case we need to access it again
