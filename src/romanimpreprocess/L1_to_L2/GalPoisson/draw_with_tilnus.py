...
import math
from scipy.stats import gamma, invgamma, betaprime, t
from scipy.stats import beta as sp_beta
from scipy.special import beta as EB
from scipy.special import betaln, loggamma, gammainc, gammaincc, logsumexp
...
def draw_from_Pearson(tilnu_21: float, tilnu_31: float, tilnu_41: float,
    I: np.ndarray, *, atol: float = 0.0, rng=None):
    """
...
def random_from_type1(tilnu_21, tilnu_31, tilnu_41, I_arr, rng=None):
    ...
    a_arr, b_arr, mu_arr, c_arr = solve_beta_parameters_vec(
        tilnu_21, tilnu_31, tilnu_41, I_arr
    )
...
def _log_k(m: float, nu: float, a: float = 1.0):
    ...
    '''
    return ((2*m - 2)*_LOG2
            + 2*loggamma(m + 0.5j*nu).real
            - (_LOGPI + math.log(a) + loggamma(2*m - 1)))
...
def _branch_masses(m: float) -> Tuple[float, float, float]:
    ...
    log_P_right = (2.0*m) - (2.0*m - 1.0)*np.log(2.0*m) + loggamma(a) + _log_reg_lower_gamma(a, 2.0*m)
...
def _log_fS_from_s(s: float, m: float, nu: float,
                   theta: float, root: float,
                   logk: float, log_dxds: float) -> float:
    """
...
def pt4_rvs_devroye(m: float, nu: float,
            *, a: float = 1.0, lam: float = 0.0,
            size=None, rng=None):
    """
    ...
    def _single():
        while True:
            ...
            s = 0
            if x > 2:  x, s = x-2, 1
            if x > 1:  z, x = math.log(x-1), 1 - math.log(x-1)
            x = (M + rc*x) if s else (M - rc*x)
            if abs(x) >= math.pi/2:           continue
    ...
    for i in range(out.size): out[i] = _single()
    return out.reshape(size)



def pt4_rvs_ar(m: float, nu: float, a: float, lam: float,
               rng: Generator | None = None) -> float:
    """
...
def random_from_type4(tilnu21: float, tilnu31: float, tilnu41: float,
                         I_array, *, devroye_threshold=0.005, rng=None):
    """
    ...
    acc_pred = np.array([devroye_acc_rate(nu_i, a_i, m_i) for nu_i, a_i, m_i in zip(nu, a, m)])
...
def random_from_type5(tilnu_21, tilnu_31, I_arr, rng=None):
    ...
    a_arr, b_arr, mu_arr = solve_pearson5_parameters_vec(
        tilnu_21, tilnu_31, I_arr
    )
