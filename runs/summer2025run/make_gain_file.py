...
infiles = open(summaries).readlines()
...
tree = {'roman': {
    ...
    'data': gain_unpack,
    'dq': np.where(good_unpack,0,2**19).astype(np.uint32)  # simple quality flag for bookkeeping for now
},
'notes': {
    'solid_waffle_config': config_lines
}
}
...
Kernel_good = (convolve(np.where(good_unpack,0,1), np.zeros((3,3)), mode='same', method='direct')<.5)[4:-4,4:-4]
...
for dy in range(-1,2):
    for dx in range(-1,2):
       if dy<0: Kernel[1+dy,1+dx,:-dy,:] = 0.
       if dy>0: Kernel[1+dy,1+dx,-dy:,:] = 0.
       if dx<0: Kernel[1+dy,1+dx,:,:-dx] = 0.
       if dx>0: Kernel[1+dy,1+dx,:,-dx:] = 0.
...
for j in range(4):
    ...
    KernelSym = (Kernel[1+dy,1+dx,ymin:ymax,xmin:xmax] + Kernel[1-dy,1-dx,ymin+dy:ymax+dy,xmin+dx:xmax+dx])/2.
...
Kernel[1,1,:,:] = 0.
Kernel[1,1,:,:] = 1. - np.sum(Kernel, axis=(0,1))
...
fits.HDUList([
  fits.PrimaryHDU(),
  fits.ImageHDU(np.transpose(tree['roman']['data'], axes=(0,2,1,3)).reshape((3*(nside-8),3*(nside-8))))
