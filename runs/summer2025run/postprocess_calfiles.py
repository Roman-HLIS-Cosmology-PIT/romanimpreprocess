...
from romanimpreprocess.pars import g_ideal  # get the ideal gain
from romanimpreprocess.utils import ipc_linearity
from romanimpreprocess.utils.sky import medfit
...
dq |= np.where(pflat < 0.01, 1, 0).astype(np.uint32)
dq |= np.where(pflat > 1.99, 1, 0).astype(np.uint32)
pflat = np.clip(pflat, 0.01, 1.99)
...
with open(f"linearity_pars_{sca:02d}.json") as f:
    lpars = json.load(f)
tframe = 3.04
if "TFRAME" in lpars:
...
with asdf.open(infile.replace("_linearitylegendre_", "_dark_")) as fd:
    ...
    dark = (
        fd["roman"]["dark_slope"][nb:-nb, nb:-nb] * tframe
    )  # reference pixels trimmed, converted to DN/frame
    (ny, nx) = np.shape(dark)
    Sdark_predicted = np.zeros((ngrp, ny, nx), dtype=np.float32)
    xref = (int(readpattern[2 * bframe]) + int(readpattern[2 * bframe + 1]) - 1) / 2.0
    print("-----", xref)
    for j in range(ngrp):
