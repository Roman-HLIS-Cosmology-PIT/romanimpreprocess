...
for j in range(1,101):
  procstring = indir + '/Total_Noise_exp{:d}_'.format(j)
  ...
  de = 0.
  for k in range(1,N):
    slp[0,:,:] = slp[0,:,:] + cube[0,k,:,:]*(k-N/2.)
    de = de + (k-N/2.)**2
  slp[0,:,:] = slp[0,:,:]/de
  de = 0.
  for k in range(1,N//2):
    slp[1,:,:] = slp[1,:,:] + cube[0,k,:,:]*(k-(N//2)/2.)
    de = de + (k-(N//2)/2.)**2
  slp[1,:,:] = slp[1,:,:]/de

  # write to a FITS file
  outfile = sys.argv[3]+'/99999999_SCA{:02d}_Noise_{:03d}.fits'.format(sca, j)
  print('>>', outfile)
  hdr = fits.Header()
