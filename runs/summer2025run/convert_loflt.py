"""
Low-intensity flat conversion script.
...
for j in range(1, 51):
    procstring = indir + f"/Gain_exp{j:d}_"
    ...
    de = 0.0
    for k in range(1, Nc):
        slp[0, :, :] = slp[0, :, :] + cube[0, k, :, :] * (k - Nc / 2.0)
        de = de + (k - Nc / 2.0) ** 2
    slp[0, :, :] = slp[0, :, :] / de
    de = 0.0
    for k in range(1, Nc // 2):
        slp[1, :, :] = slp[1, :, :] + cube[0, k, :, :] * (k - (Nc // 2) / 2.0)
        de = de + (k - (Nc // 2) / 2.0) ** 2
    slp[1, :, :] = slp[1, :, :] / de

    # write to a FITS file
    outfile = sys.argv[3] + f"/99999999_SCA{sca:02d}_LoFlat_{j:03d}.fits"
    print(">>", outfile)
    hdr = fits.Header()
    hdr["PROVEN"] = ("convert_loflt.py", "Conversion script")
